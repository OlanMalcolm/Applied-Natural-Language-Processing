...
class Classifier(object):
    ...
    def read_dask_dataframe(self, filePath):
        ...
        """
        df = dd.read_csv(filePath, names = ['sentiment', 'title', 'text'], header = 0)
        return df
    ...
    def combine_text(self, df):
        ...
        df = df.assign(content = df['title'] + " " + df['text'])
        return df
    ...
    def compute_preprocess_parallel(self, df):
        ...
        df['text_processed'] = df['content'].map(self.preprocess_text, meta = ('text_processed', 'str'))
        return df.compute()
    ...
    def init_XGBoost(self):
        ...
        clf = XGBClassifier(n_jobs = 4, use_label_encoder = False, eval_metrics = 'logloss')
        return clf
