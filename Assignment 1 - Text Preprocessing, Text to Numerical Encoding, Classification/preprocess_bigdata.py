...
class Preprocess:
    ...
    def tokenize(self, batch, max_length=100):
        ...
        '''
        # 5.1.2
        tokenized = self.tokenizer(batch['text'], padding = 'max_length', truncation = True, max_length = max_length)
        tokens_list = [self.tokenizer.convert_ids_to_tokens(ids) for ids in tokenized['input_ids']]
        tokens_dict = {"tokens" : tokens_list}
        return tokens_dict
    ...
    def preprocess_text(self):
        ...
        '''
        #5.1.3
        dataset_cleaned = self.dataset.map(
            self.tokenize,
            batched=True,
            batch_size=1000,
        ...
        lambda x: {'id': x['id'], 'title': x['title'], 'tokens': x['tokens']}
        )
